...
public class TextDataLoaderTests
{
    ...
    {
        var vdb = new MockChunkingVectorDatabase();
        ...
        var texts = vdb.InternalVectorStore.ToArray();

        Assert.AreEqual(9, texts.Length);
        Assert.AreEqual("the lion king is a", texts[0].Value.Text);
        Assert.AreEqual("is a 1994 disney animated", texts[1].Value.Text);
        Assert.AreEqual("disney animated film about a", texts[2].Value.Text);
    }




    // Dummy implementation that satisfies IVectorDatabase<int, string> and IVectorDatabase<int, string, string>
    public class MockChunkingVectorDatabase : MemoryVectorDatabase<string>
    {
        // property to expose base internal VectorStore
        public MemoryDictionaryVectorStoreWithVocabulary<int, string, Build5Nines.SharpVector.Vocabulary.DictionaryVocabularyStore<string>, string, int> InternalVectorStore => base.VectorStore;
    }
}